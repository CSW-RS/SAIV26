...
The shape of the output tensor $Y$ is determined by the shape of the input tensor $X$, the shape of the kernel $W$ and the values of attributes $pads$, $dilations$ and $strides$.

$$
Y_\textbf{shape} = \left(dX_0, \frac{dW_0}{groups}, dY_2, dY_3\right)
$$

Where:

- $dY_2 =  \displaystyle\left\lfloor{\frac{\alpha-\theta}{strides[0]}} \right\rfloor + 1$

    <br>

    - $\displaystyle\alpha = dX_2 + pads[0] + pads[2]$

    <br>

    - $\displaystyle\theta = dilations[0] * (dW_2 - 1) + 1$

<br>

- $dY_3 =  \displaystyle\left\lfloor{\frac{\beta-\gamma}{strides[1]}} \right\rfloor + 1$

    <br>

    - $\displaystyle\beta = dX_3 + pads[1] + pads[3]$

    <br>

    - $\displaystyle\gamma = dilations[1] * (dW_3 - 1) + 1$
